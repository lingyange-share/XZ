...
<script type="text/javascript">
		...
		window.location.href="listServiceBlue.action?pageNO="+page.value;
	}
</script>
</head>
...
<div class="nav">
    <div class="navbox">
       <ul id="bannerUlId"> 
            ...
            <li><img src="${pageContext.request.contextPath }/images/society/device.png" align="absmiddle" /></li>
            <li class="nav_block current"><a href="listServiceBlue.action">会员服务</a></li>
            <li><img src="${pageContext.request.contextPath }/images/society/device.png" align="absmiddle" /></li>
...
<div class="position">您所在位置：会员服务</div>
...
<c:forEach items="${page.item}" var="c">
	...
	<fmt:formatDate var="time" value="${t}" pattern="yyyy-MM-dd"/>
      <div class="any_list clearfix">
        <ul>
          <li class="li_bottom">
          <a href="findServiceById.action?id=${c.id}">${c.title}</a>
          <span>[${time}]</span>
          </li>
        </ul>
      </div>
    ...
    共${page.totalCount}条记录 每页显示${page.pageSize }条 当前 ${page.pageNO }/${page.totalPage } 页  
       <a href='listServiceBlue.action?pageNO=1'>首页</a>
       <a href='listServiceBlue.action?pageNO=${page.pageNO-1 }'>上一页</a>
       <a href='listServiceBlue.action?pageNO=${page.pageNO}' >${page.pageNO}</a>
       <c:forEach var="i" begin="${page.pageNO+1}" end="${page.pageNO+4}">
       <a href='listServiceBlue.action?pageNO=${i}' >${i}</a>
       </c:forEach>
       <a href='listServiceBlue.action?pageNO=${page.pageNO+1}' >...</a>
       <a href='listServiceBlue.action?pageNO=${page.pageNO+1 }' >下一页</a> 
       <a href='listServiceBlue.action?pageNO=${page.totalPage }' >尾页</a>
